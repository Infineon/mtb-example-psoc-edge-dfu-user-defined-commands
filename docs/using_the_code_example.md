...
The following example clones the "[dfu-user-defined-commands](https://github.com/Infineon/mtb-example-psoc-edge-dfu-user-defined-commands)" application with the desired name "DfuUserCommand" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge--dfu-user-defined-commands --user-app-name DfuUserCommand --target-dir "C:/mtb_projects"
   ```
...
> **Note:** Ensure MiniProg4 (external) is disconnected from the Host PC/Device while Programming the device.
...
> **Note:** Ensure MinProg4 (external) is disconnected from the Host PC/device while you try to attach the debugger.
